...
class GolemModel(nn.Module):
    ...
    def __init__(self, n, d, lambda_1, lambda_2,l1, l1_quad, equal_variances,
    ...
    def forward(self, X):
        ...
        self.likelihood, self.quad = self._compute_likelihood()
        self.L1_penalty = self._compute_L1_penalty()
        self.h = self._compute_h()
        self.score = (self.likelihood + self.l1 * self.L1_penalty)
    ...
    def _compute_likelihood(self):
        ...
        if self.equal_variances:  # Assuming equal noise variances
            return (0.5 * self.d *torch.log(torch.square(torch.linalg.norm(self.X - self.X @ self.B))) - torch.linalg.slogdet(torch.eye(self.d).to(self.device) - self.B)[1]), 0.5 / self.d * torch.square(torch.linalg.norm(self.X - self.X @ self.B)) + self.l1_quad*torch.linalg.matrix_norm(self.B, ord=1)
